<script setup lang="ts">
	...
	const currentMonth = computed(() => shortMonths[selectedMonth.value]);
	const currentYear = computed(() => selectedYear.value);

	const dayInMonth = computed(() => new Date(selectedYear.value, selectedMonth.value + 1, 0).getDate());
		...
		<div class="flex items-center justify-center gap-4">
			...
			<div class="flex w-45.5 items-center justify-center gap-2">
				<button class="flex items-center gap-1 rounded-lg border border-[#d9d9d9] p-1.5">
					<p class="w-12.75 text-left">{{ currentMonth }}</p>
